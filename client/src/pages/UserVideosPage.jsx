...
export default function UserVideosPage() {
  ...
  // Optional: Fetch user-specific video count or stats if needed
  useEffect(() => {
    // You can fetch user specific videos here or let VideoStudio handle it
  }, [user]);
  ...
  return (
    <div className="bg-[#09090b] min-h-screen text-zinc-100 pb-16 selection:bg-indigo-500 selection:text-white">
      {/* Channel Banner with Subtle Gradient & Glow */}
      ...
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className="flex flex-col md:flex-row items-center md:items-start gap-6 -mt-12 relative z-10 pb-8 border-b border-zinc-800/80">
          ...
          <div className="flex items-center gap-3 pt-4 md:pt-2">
            <button className="flex items-center gap-2 bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-500 hover:to-violet-500 text-white px-4 py-2 rounded-xl text-xs font-semibold shadow-lg shadow-indigo-500/20 transition duration-300">
              <Sparkles className="w-3.5 h-3.5" /> Customize Channel
            </button>
            <button className="p-2 rounded-xl bg-zinc-900 border border-zinc-800 text-zinc-400 hover:text-zinc-200 hover:border-zinc-700 transition">
        ...
        <div className="flex items-center gap-6 pt-4 border-b border-zinc-800/60 mb-6 text-xs font-semibold">
          <button className="pb-3 border-b-2 border-indigo-500 text-indigo-400 flex items-center gap-2">
            <Video className="w-4 h-4" /> Videos & Uploads
          </button>
          <button className="pb-3 border-b-2 border-transparent text-zinc-400 hover:text-zinc-200 transition">
            Analytics
          </button>
          <button className="pb-3 border-b-2 border-transparent text-zinc-400 hover:text-zinc-200 transition">
            Playlists
